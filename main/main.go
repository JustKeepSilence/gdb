...
func main() {
	...
	dbPath, itemDbPath, authorization, logWriting, level := dbConfigs.DbPath, dbConfigs.ItemDbPath, dbConfigs.Authorization, dbConfigs.LogConfigs.LogWriting, dbConfigs.LogConfigs.Level
	if dbConfigs.Mode == "" {
		dbConfigs.Mode = "restful"
	}
	if dbConfigs.Mode != "restful" && dbConfigs.Mode != "rpc" {
	...
	if dbConfigs.Mode == "restful" {
		if err := db.InitialDbServer(dbIp, dbPort, dbPath, itemDbPath, startReadConfigTime, authorization, logWriting, level); err != nil {
			log.Print(fmt.Errorf("System initialization failed: " + err.Error()))
			time.Sleep(60 * time.Second)
			return
		}
