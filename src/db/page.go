...
// user login
func (ldb *LevelDb) userLogin(info authInfo) error {
	userName := info.UserName
	v, err := ldb.InfoDb.Get([]byte(userName), nil)
	if err != nil {
...
func (ldb *LevelDb) getUserInfo(userName string) (map[string]interface{}, error) {
	v, err := ldb.InfoDb.Get([]byte(userName), nil)
	if err != nil {
